...
Interpret these states through the selected mode:

- `source-faithful`: object physics and source-specific verbs lead. Preserve the source silhouette and structure through every action; use bubbles, wobble, crumbs, roll, steam, melt, light, or another real property for `signature` and `play`.
- `flavor-monster`: approved anatomy leads. Let ears, horns, tuft, tail, belly markings, or ingredient features supply secondary motion; let `rest` curl or puddle according to body type; let `signature` express one flavor-DNA event; let `play` use an internal feature or appendage rather than a generic prop.
