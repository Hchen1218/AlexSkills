...
桌宠模式提供两种并列形象：写实卡通桌宠保留原主体轮廓与关键结构；风味小怪兽桌宠提取颜色、配料、材质和动作特征，重新设计成独立怪兽。两者共用透明动作资产、角色包以及启动/关闭入口。
...
- **桌宠模式**：先选择写实卡通或风味小怪兽，完成形象确认与动作确认后，导出相同格式的可运行透明角色包。
...
没有可识别主体时，Skill 会要求重新上传；出现多个独立主体时，会先编号请用户选择。杯中饮料与碗中食物默认按“容器 + 内容物”整体识别。用户没有明确形象模式时，Skill 必须先询问选择写实卡通或风味小怪兽，不能自行默认。三款候选必须是角色设计差异，而不是同一角色只换动作。

- **写实卡通桌宠**：照片主体就是角色身体，保留外轮廓、关键分区与物体特有动作。
- **风味小怪兽桌宠**：照片只提供色彩、配料、材质与动作 DNA；容器不能继续充当身体，动作改由怪兽的耳、角、尾巴、肚皮斑纹等结构表达。

两种模式仅改变形象和动作设定，不改变十二状态名称、运行器、schema v2、审批门槛或交付方式。
