...
Desktop-pet mode has two parallel character modes. Neither is a fallback for the other, and neither changes the runtime, schema-v2 pack, approval gates, or delivery format.
...
- `idle` should reveal creature life through breathing, blink, ear/tuft twitch, or a small internal ingredient shift.
- `rest` should curl, puddle, tuck, or soften according to the approved body type.
- `curious`, `stretch`, and `tiptoe` should use the approved monster anatomy instead of mimicking a source container.
- `signature` should express flavor DNA through one restrained event: a fruit-cube hiccup, one bubble, a crumb, a color ripple, a leaf-ear shake, or another approved verb.
- `play` should interact with an internal feature or appendage, not introduce a generic toy or unrelated prop.
- `drag`, `fall`, and `land` must preserve the monster silhouette and connect physically; loose ears, horns, tufts, and tails may lag or settle, but limb inventory must remain stable.
...
- the same twelve required actions and optional `fall` / `touch` extensions;
- the same 120–140 px readability target, transparent runtime strips, schema-v2 manifest, builder, validator, and delivery folder.

Mode changes visual identity and action interpretation only. Do not fork the pack schema or duplicate the Electron runtime.
